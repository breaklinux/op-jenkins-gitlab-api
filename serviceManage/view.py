...
@serviceUrl.route('/api/v1', methods=['GET', 'POST'])
def serviceRun():
    import requests, json
    from tools.config import ansibleApiUrl,channelID
    from tools.service_log import serviceLogInster
    if request.method == "GET":
        ...
        if env in serviceEnvlistOld:
            if service_operation in ops_oper:
                ...
                serviceResult = requests.post(ansibleApiUrl, data=json.dumps(serviceData), headers={"Content-Type": "application/json"})
...
def qeuryServiceHistrory():
    ...
    from tools.config import serviceMgHeader
...
def dataResult(Data):
    ...
    if Data:
        ...
        msg = "未查询到数据"
    return Response(json.dumps({"code": 0, "total": len(data_list),"data":data_list,"msg":msg,"columns":serviceMgHeader}),mimetype='application/json')
